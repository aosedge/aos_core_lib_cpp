...
/**
 * Max number of alert processors for instance.
 */
static constexpr auto cMaxAlertProcessorsPerInstance
    = AOS_CONFIG_MONITORING_ALERT_PROCESSOR_SYSTEM_RESOURCES_SIZE + cMaxNumPartitions;
...
struct ResourceIdentifier {
    ...
    ResourceType                              mType;
    Optional<StaticString<cPartitionNameLen>> mName;
    ...
    friend Log& operator<<(Log& log, const ResourceIdentifier& identifier)
    {
        ...
        if (identifier.mName.HasValue()) {
            log << ":" << identifier.mName.GetValue();
        }
