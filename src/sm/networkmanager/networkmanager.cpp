    ...
    const String& instanceID, const String& networkID, const NetworkParams& network)
{
    ...
    StaticArray<StaticString<cHostNameLen>, cMaxNumHosts * cMaxNumInstances> mHost;

    if (err = PrepareCNIConfig(instanceID, networkID, network, netConfigList, rtConfig, mHost); !err.IsNone()) {
        return err;
    }
    ...
    if (err = UpdateInstanceNetworkCache(instanceID, networkID, network.mNetworkParameters.mIP, mHost); !err.IsNone()) {
        return err;
    }
    ...
    Array<StaticString<cHostNameLen>>& hosts) const
{
    ...
    if (!network.mInstanceIdent.mServiceID.IsEmpty() && !network.mInstanceIdent.mSubjectID.IsEmpty()) {
        ...
        host.Format("%d.%s.%s", network.mInstanceIdent.mInstance, network.mInstanceIdent.mSubjectID,
            network.mInstanceIdent.mServiceID);
        ...
        if (network.mInstanceIdent.mInstance == 0) {
            host.Format("%s.%s", network.mInstanceIdent.mSubjectID, network.mInstanceIdent.mServiceID);
