...
static constexpr auto cOSSLMaxNameSize = 50; // taken from openssl sources: include/internal/sizes.h
...
Error SetRSAPubKey(const EVP_PKEY* src, Variant<ECDSAPublicKey, RSAPublicKey>& dst)
{
    ...
    auto modulus = DeferRelease(static_cast<uint8_t*>(OPENSSL_zalloc(modSize)), AOS_OPENSSL_free);
    if (BN_bn2bin(n, modulus.Get()) < 0) {
        return AOS_ERROR_WRAP(ErrorEnum::eFailed);
    }

    auto exponent = DeferRelease(static_cast<uint8_t*>(OPENSSL_zalloc(expSize)), AOS_OPENSSL_free);
...
Error ConvertX509ToPEM(X509* cer, String& pem)
{
...
RetWithError<EVP_PKEY*> GetEvpPublicKey(const RSAPublicKey& pubKey)
{
    ...
    auto params = DeferRelease(OSSL_PARAM_BLD_to_param(bld.Get()), AOS_OPENSSL_free);
    if (!params) {
        return {nullptr, OPENSSL_ERROR()};
    }
...
Error SetPublicKey(const PublicKeyItf& pubKey, X509_REQ* csr)
{
    Error     err;
    EVP_PKEY* pkey = nullptr;
    ...
    case KeyTypeEnum::eRSA:
        Tie(pkey, err) = GetEvpPublicKey(static_cast<const RSAPublicKey&>(pubKey));
        break;
    case KeyTypeEnum::eECDSA:
        Tie(pkey, err) = GetEvpPublicKey(static_cast<const ECDSAPublicKey&>(pubKey));
        break;
    default:
        return AOS_ERROR_WRAP(ErrorEnum::eInvalidArgument);
    }
...
Error SetPublicKey(const PublicKeyItf& pubKey, X509* cer)
{
    Error     err;
    EVP_PKEY* pkey = nullptr;
...
aos::UniquePtr<X509_NAME, void (&)(X509_NAME* a)> ConvertDER2X509Name(const Array<uint8_t>& der)
{
...
RetWithError<EVP_MD_CTX*> CreateSignCtx(const PrivateKeyItf& privKey)
{
    const char* cAosProps = "provider=" AOS_SIGNER;

    // Create EVP_PKEY
    auto pKeyCtx = DeferRelease(EVP_PKEY_CTX_new_from_name(NULL, AOS_ALGORITHM, cAosProps), EVP_PKEY_CTX_free);
    if (!pKeyCtx) {
        return {nullptr, OPENSSL_ERROR()};
    }
    ...
    OSSL_PARAM     privKeyParams[]
        = {OSSL_PARAM_octet_string(PKEY_PARAM_AOS_KEYPAIR, reinterpret_cast<void*>(privKeyPtr), sizeof(privKeyPtr)),
            OSSL_PARAM_END};
    ...
    // Sign
    auto mdCtx = DeferRelease(EVP_MD_CTX_create(), EVP_MD_CTX_free);

    // Use default MD algorithm based on the specified private key.
    if (EVP_DigestSignInit_ex(mdCtx.Get(), nullptr, nullptr, nullptr, cAosProps, evpKey, nullptr) != 1) {
        return {nullptr, OPENSSL_ERROR()};
    }
...
Error Sign(const PrivateKeyItf& privKey, X509* cer)
{
    auto [signCtx, err] = CreateSignCtx(privKey);
    if (!err.IsNone()) {
        return err;
    }
...
Error CreateClientCert(
    X509_REQ* csr, EVP_PKEY* caKey, X509* caCert, const Array<uint8_t>& serial, String& clientCertPEM)
{
    auto clientCert = DeferRelease(X509_new(), X509_free);
    if (!clientCert) {
        return OPENSSL_ERROR();
    }
    ...
    return ConvertX509ToPEM(clientCert.Get(), clientCertPEM);
}
...
Error Sign(const PrivateKeyItf& privKey, X509_REQ* req)
{
    auto [signCtx, err] = CreateSignCtx(privKey);
    if (!err.IsNone()) {
        return err;
...
Error OpenSSLCryptoProvider::Init()
{
    ...
    if (auto err = mOpenSSLProvider.Load(); !err.IsNone()) {
        return err;
    }
    ...
    const x509::Certificate& templ, const x509::Certificate& parent, const PrivateKeyItf& privKey, String& pemCert)
{
    ...
    auto cert = DeferRelease(X509_new(), X509_free);
    if (!cert) {
        return OPENSSL_ERROR();
    }

    if (auto err = SetPublicKey(privKey.GetPublic(), cert.Get()); !err.IsNone()) {
        return err;
    }
    ...
    if (auto err = Sign(privKey, cert.Get()); !err.IsNone()) {
        return err;
    }

    if (auto err = ConvertX509ToPEM(cert.Get(), pemCert); !err.IsNone()) {
        return err;
    }
    ...
    const Array<uint8_t>& serial, String& clientCert)
{
    ...
    auto caKey = DeferRelease(PEM_read_bio_PrivateKey(bioCaKey.Get(), nullptr, nullptr, nullptr), EVP_PKEY_free);
    if (!caKey) {
        return ErrorEnum::eFailed;
    }
    ...
    return aos::crypto::CreateClientCert(csr.Get(), caKey.Get(), caCert.Get(), serial, clientCert);
}
...
Error OpenSSLCryptoProvider::CreateCSR(const x509::CSR& templ, const PrivateKeyItf& privKey, String& pemCSR)
{
    ...
    auto csr = DeferRelease(X509_REQ_new_ex(nullptr, nullptr), X509_REQ_free);
    if (!csr) {
        return OPENSSL_ERROR();
    }
    ...
    err = SetPublicKey(privKey.GetPublic(), csr.Get());
    if (!err.IsNone()) {
        return err;
    }

    err = Sign(privKey, csr.Get());
    if (!err.IsNone()) {
        return err;
    }
...
Error OpenSSLCryptoProvider::ASN1DecodeDN(const Array<uint8_t>& dn, String& result)
{
    ...
    auto buf = DeferRelease(X509_NAME_oneline(name.Get(), 0, 0), AOS_OPENSSL_free);
    ...
    // To be compatible with MBedTLS implementation replace separators with ", " and remove leading "/"
...
Error OpenSSLCryptoProvider::ASN1EncodeBigInt(const Array<uint8_t>& number, Array<uint8_t>& asn1Value)
{
    ...
    auto releaseBuf = DeferRelease(buf, AOS_OPENSSL_free);
...
Error OpenSSLCryptoProvider::OpenSSLHash::Init(const EVP_MD* type)
{
    mCtx = EVP_MD_CTX_new();
    if (!mCtx) {
        return OPENSSL_ERROR();
    ...
    if (EVP_DigestInit_ex(mCtx, mType, nullptr) != 1) {
        return OPENSSL_ERROR();
    }
...
{
    if (EVP_DigestUpdate(mCtx, data.Get(), data.Size()) != 1) {
        return OPENSSL_ERROR();
    }
...
Error OpenSSLCryptoProvider::OpenSSLHash::Finalize(Array<uint8_t>& hash)
{
    ...
    if (EVP_DigestFinal_ex(mCtx, hash.Get(), &size) != 1) {
        return OPENSSL_ERROR();
    }
