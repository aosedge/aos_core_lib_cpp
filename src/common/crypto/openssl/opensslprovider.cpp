...
const char* KeyMgmtQuery(int id)
{
    ...
    return AOS_ALGORITHM;
}

int KeyMgmtImport(void* keydata, int selection, const OSSL_PARAM* p)
{
    if (selection != EVP_PKEY_KEYPAIR) {
        LOG_ERR() << AOS_ERROR_WRAP(Error(ErrorEnum::eFailed, "Not supported selection for AOS key"));
    ...
    const OSSL_PARAM* pKeyParam = OSSL_PARAM_locate_const(p, PKEY_PARAM_AOS_KEYPAIR);
    if (pKeyParam != nullptr) {
        if (pKeyParam->data_type != OSSL_PARAM_OCTET_STRING) {
            LOG_ERR() << AOS_ERROR_WRAP(Error(ErrorEnum::eFailed, "Wrong data type for AOS key"));

            return 0;
        }

        if (pKeyParam->data_size != sizeof(const PrivateKeyItf*)) {
            LOG_ERR() << AOS_ERROR_WRAP(Error(ErrorEnum::eFailed, "Wrong data size for AOS key"));
...
const OSSL_PARAM* KeyMgmtImportTypes(int selection)
{
    ...
    static const OSSL_PARAM cAosImportTypes[] = {OSSL_PARAM_BN(PKEY_PARAM_AOS_KEYPAIR, NULL, 0), OSSL_PARAM_END};
...
X509_ALGOR* GetSignAlg(const PublicKeyItf& pubKey)
{
    ...
    case KeyTypeEnum::eRSA: {
        ...
        // According to ossl_DER_w_algorithmIdentifier_MDWithRSAEncryption implementation:
        // PARAMETERS, always NULL in current standards
...
int DgstSignInit(void* ctx, const char* mdname, void* provkey, const OSSL_PARAM params[])
{
    ...
    if (!aosCtx->mAosKey || !aosCtx->mAosKey || !aosCtx->mAosKey->mPrivKey) {
        LOG_ERR() << "AOS context is not initialized, err=" << AOS_ERROR_WRAP(Error(ErrorEnum::eInvalidArgument));
...
int DgstSign(void* ctx, unsigned char* sig, size_t* siglen, size_t sigsize, const unsigned char* tbs, size_t tbslen)
{
    ...
    if (!ctx || !siglen || !tbs) {
        LOG_ERR() << AOS_ERROR_WRAP(Error(ErrorEnum::eInvalidArgument));
...
const OSSL_ALGORITHM* ProviderQuery(void* provctx, int operationID, int* noCache)
{
    ...
    *noCache = 0;
    ...
    static const OSSL_ALGORITHM cSignAlgorithms[] = {
        {AOS_ALGORITHM ":aosEncryption", "provider=" AOS_SIGNER, cSignFunctions, "AOS Signature"}, OSSL_ALGORITHM_END};
    ...
    static const OSSL_ALGORITHM cKeyMgmAlgorithms[]
        = {{AOS_ALGORITHM ":aosEncryption", "provider=" AOS_SIGNER, cKeyMgmFunctions, "AOS Key Management"},
            OSSL_ALGORITHM_END};
...
Error OpenSSLProvider::Load()
{

    if (CONF_modules_load(nullptr, nullptr, 0) != 1) {
        return OPENSSL_ERROR();
    }

    mDefaultProvider = OSSL_PROVIDER_load(nullptr, "default");
    if (!mDefaultProvider) {
        return OPENSSL_ERROR();
    }

    if (OSSL_PROVIDER_add_builtin(nullptr, AOS_SIGNER, &ProviderInit) != 1) {
        return OPENSSL_ERROR();
    }

    mProvider = OSSL_PROVIDER_load(nullptr, AOS_SIGNER);
    if (!mProvider) {
        return OPENSSL_ERROR();
    }

    if (EVP_set_default_properties(NULL, "?provider!=" AOS_SIGNER) != 1) {
        return OPENSSL_ERROR();
    }
...
{
    if (mProvider) {
        if (OSSL_PROVIDER_unload(mProvider) != 1) {
            return OPENSSL_ERROR();
        }
        mProvider = nullptr;
    }
