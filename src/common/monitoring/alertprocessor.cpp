...
public:
    ...
    Res Visit(const cloudprotocol::SystemAlert&) const { return {}; }

    Res Visit(const cloudprotocol::CoreAlert&) const { return {}; }

    Res Visit(const cloudprotocol::DeviceAllocateAlert&) const { return {}; }

    Res Visit(const cloudprotocol::ResourceValidateAlert&) const { return {}; }

    Res Visit(const cloudprotocol::DownloadAlert&) const { return {}; }

    Res Visit(const cloudprotocol::ServiceInstanceAlert&) const { return {}; }
...
Error AlertProcessor::HandleMaxThreshold(uint64_t currentValue, const Time& currentTime)
{
    ...
    if (currentValue >= mMaxThreshold && mMaxThresholdTime.IsZero()) {
        LOG_DBG() << "Max threshold crossed: id=" << mID << ", maxThreshold=" << mMaxThreshold
    ...
    if (currentValue >= mMaxThreshold && !mMaxThresholdTime.IsZero()
        ...
        LOG_DBG() << "Resource alert: id=" << mID << ", value=" << currentValue << ", status=" << status
                  << ", time=" << currentTime;
...
Error AlertProcessor::HandleMinThreshold(uint64_t currentValue, const Time& currentTime)
{
    ...
    if (currentValue <= mMinThreshold && mMinThresholdTime.IsZero()) {
        LOG_DBG() << "Min threshold crossed: id=" << mID << ", value=" << currentValue
...
Error AlertProcessor::SendAlert(uint64_t currentValue, const Time& currentTime, cloudprotocol::AlertStatus status)
{
    ...
    if (auto err = mAlertSender->SendAlert(alert); !err.IsNone()) {
        LOG_ERR() << "Failed to send alert: " << err;
