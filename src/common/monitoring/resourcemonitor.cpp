...
{
    if (id.mType == ResourceTypeEnum::ePartition && id.mName.HasValue()) {
        return id.mName.GetValue();
    }
...
Error ResourceMonitor::SetupSystemAlerts(const NodeConfig& nodeConfig)
{
    ...
    if (alertRules.mCPU.HasValue()) {
        ...
        if (auto err = alertProcessor.Init(
                id, mMaxDMIPS, alertRules.mCPU.GetValue(), *mAlertSender, CreateSystemQuotaAlertTemplate(id));
            !err.IsNone()) {
            return AOS_ERROR_WRAP(err);
        }

        if (auto err = mAlertProcessors.PushBack(alertProcessor); !err.IsNone()) {
            return AOS_ERROR_WRAP(err);
        }
    }
    ...
    for (const auto& partitionRule : alertRules.mPartitions) {
        ...
        if (err = alertProcessor.Init(id, totalSize, partitionRule, *mAlertSender, CreateSystemQuotaAlertTemplate(id));
            !err.IsNone()) {
            return AOS_ERROR_WRAP(err);
        }

        if (err = mAlertProcessors.PushBack(alertProcessor); !err.IsNone()) {
            return AOS_ERROR_WRAP(err);
        }
    }
    ...
    if (alertRules.mUpload.HasValue()) {
        ...
        if (auto err
            = alertProcessor.Init(id, alertRules.mUpload.GetValue(), *mAlertSender, CreateSystemQuotaAlertTemplate(id));
            !err.IsNone()) {
            return AOS_ERROR_WRAP(err);
        }
...
Error ResourceMonitor::SetupInstanceAlerts(const String& instanceID, const InstanceMonitorParams& instanceParams)
{
    ...
    if (auto err = mInstanceAlertProcessors.Set(instanceID, {}); !err.IsNone()) {
        return AOS_ERROR_WRAP(err);
    }
    ...
    if (alertRules.mCPU.HasValue()) {
        ...
        if (auto err = alertProcessor.Init(id, mMaxDMIPS, alertRules.mCPU.GetValue(), *mAlertSender,
                CreateInstanceQuotaAlertTemplate(instanceIdent, id));
            !err.IsNone()) {
            return AOS_ERROR_WRAP(err);
        }

        if (auto err = alertProcessors.PushBack(alertProcessor); !err.IsNone()) {
            return AOS_ERROR_WRAP(err);
        }
    }
    ...
    if (alertRules.mRAM.HasValue()) {
        ...
        if (auto err = alertProcessor.Init(id, mMaxMemory, alertRules.mRAM.GetValue(), *mAlertSender,
                CreateInstanceQuotaAlertTemplate(instanceIdent, id));
            !err.IsNone()) {
            return AOS_ERROR_WRAP(err);
        }
    ...
    if (alertRules.mDownload.HasValue()) {
        ...
        if (auto err = alertProcessor.Init(id, alertRules.mDownload.GetValue(), *mAlertSender,
                CreateInstanceQuotaAlertTemplate(instanceIdent, id));
            !err.IsNone()) {
            return AOS_ERROR_WRAP(err);
        }
...
void ResourceMonitor::ProcessMonitoring()
{
    while (true) {
        ...
        for (auto& [instanceID, instanceMonitoringData] : mInstanceMonitoringData) {
            ...
            if (auto [alertProcessor, err] = mInstanceAlertProcessors.At(instanceID); err.IsNone()) {
                ProcessAlerts(instanceMonitoringData.mMonitoringData, mNodeMonitoringData.mTimestamp, alertProcessor);
            }
    ...
    const ResourceIdentifier& id, const MonitoringData& monitoringData) const
{
    ...
    case ResourceTypeEnum::eCPU:
        return static_cast<uint64_t>(monitoringData.mCPU);
    ...
    case ResourceTypeEnum::ePartition: {
        if (!id.mName.HasValue()) {
            return {0, ErrorEnum::eNotFound};
        }

        auto [it, err] = monitoringData.mPartitions.FindIf(
            [&id](const auto& partition) { return partition.mName == id.mName.GetValue(); });
