...
SystemQuotaAlert CreateSystemQuotaAlert(const String& nodeID, const String& parameter, uint64_t value,
    std::optional<QuotaAlertStateEnum> state = std::nullopt, const Time& timestamp = Time())
{
    SystemQuotaAlert systemQuotaAlert(timestamp);
...
TEST_F(AlertProcessorTest, CheckRulePercentAlertDetection)
{
    ...
    struct {
        uint64_t                           mCurrentValue;
        Duration                           mTimeDelta;
        std::optional<QuotaAlertStateEnum> mExpectedState;
    } testCases[] = {
        {1 * maxDMIPS / 100, 0, {}},
